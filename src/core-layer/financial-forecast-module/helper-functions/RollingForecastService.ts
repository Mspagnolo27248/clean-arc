...
export class RollingForecastService {
    ...
    DailyBlendRequirements: Map<string, DailyBlendRequirementsItem>;
  ...
  constructor(params: RollingForecastInputParams) {
    ...
    this.ProductsForModelItem = this.createProductMap(params.ProductsForModelItem);
    this.Receipts = this.createReceiptsMap(params.Receipts);   
    this.DailyOpenOrders = this.createOpenOrdersMap(params.DailyOpenOrders);
    this.DailyDemandForecast = this.createDailyDemandMap(params.DailyDemandForecast);
    this.DailyBlendRequirements = this.createBlendRequirementsMap(params.DailyBlendRequirements); 
  ...
  calculateEndingInventory(
    ...
    const openInventory = previousEndInventory !== null
    ...
    const productionOut = this.ProductionOut[`${productCode}`]?.[`${date}`] || 0; // Fixed key format
    ...
    const blendRequirements = this.DailyBlendRequirements.get(`${productCode}|${date}`)?.Gals || 0;
  ...
  outputModel(): ModelOutputParams {
    ...
    return { 
        ...
        Output: result } as ModelOutputParams;
  }
...
//   }
  ...
  private createBlendRequirementsMap(params:DailyBlendRequirementsItem[]){
    return new Map(
        params.map((item) => {
          const key = this.generateStringKeyFromFields(item, [
